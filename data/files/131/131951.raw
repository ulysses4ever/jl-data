...
export zeros_gj, zeros_glj, zeros_grjm, zeros_grjp
export weights_gj, weights_glj, weights_grjm, weights_grjp
export diffmat_gj, diffmat_glj, diffmat_grjm, diffmat_grjp
export lagrange_gj, lagrange_glj, lagrange_grjm, lagrange_grjp
export interpmat_gj, interpmat_glj, interpmat_grjm, interpmat_grjp

function zgj(Q, a=0, b=0; T=Float64)
    jacobi_zeros(Q, a, b, T=T)
end

function zglj(Q, a=0, b=0; T=Float64)
    z = jacobi_zeros(Q-2, a+1, b+1, T=T)
    return [-1, z, 1]
end


function zgrjm(Q, a=0, b=0; T=Float64)
    z = jacobi_zeros(Q-1, a, b+1, T=T)
    return [-1, z]
end


function zgrjp(Q, a=0, b=0; T=Float64)
    z = jacobi_zeros(Q-1, a+1, b, T=T)
    return [z, 1]
end
...
function igj{T<:Real}(zp::Array{T,1}, z::Array{T,1}, alpha=0, beta=0)
    ...
    local Q::Int = length(z)
    local i::Int, j::Int, np::Int = length(zp)
