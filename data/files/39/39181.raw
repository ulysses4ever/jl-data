...
for key in keys(datasets)
  ...
  n_label = Nsample # int(ntoh(label_header[2]))
  data_header = read(data_f, Int32, 4)
  @assert ntoh(data_header[1]) == 2051
  n_data = Nsample # int(ntoh(data_header[2]))
  ...
  h5open("$key.hdf5", "w") do h5
    dset_data = d_create(h5, "features", datatype(Float64), dataspace(w, h, n_data))
    dset_label = d_create(h5, "label", datatype(Int64), dataspace(1,1, n_data))
    # changed from dataspace(1,1,1,n_data)
    for i = 1:n_batch
      ...
      for j = 1:length(idx)
        r_idx = rp[j]
        dset_data[:,:,idx[j]] = img[(r_idx-1)*h*w+1:r_idx*h*w]
        dset_label[1,1,idx[j]] = class[r_idx]
      end
    end
  end
