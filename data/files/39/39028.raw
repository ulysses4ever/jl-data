...
function container_add{V}(c::Deque{V}, args...)
    println("container_add deque ", args)
    v=V(args...)
    println("container_add deque created item to add")
    push!(c, v)
    v
end
...
# Given a Set
container_key(::Type{Set})=:self
container_value{V}(::Type{Set{V}})=V
container_construct{V}(::Type{Set{V}})=Set{V}()
container_push{V}(c::Set{V}, v::V)=begin push!(c, v); v end
container_add{V}(c::Set{V}, args...)=begin v=V(args...); push!(c, v); v end
container_add_key{V}(c::Set{V}, args...)=container_add(c, args...)
