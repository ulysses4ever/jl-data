...
function write_bedfile(output_path,df::DataFrame)
...
function bedtools_intersect(A::DataFrame, B::DataFrame; negate=false)

    tmpdir = tempdir()
    tmpa=joinpath( tmpdir, "A")
    tmpb=joinpath( tmpdir, "B")
    tmpc=joinpath( tmpdir, "C")
    write_bedfile(tmpa,A)
    write_bedfile(tmpb,B)
...
function bedtools_intersect(pathA::String, B::DataFrame;negate=false)
    minusv = ""
    if negate
      minusv = "-v"
    end
