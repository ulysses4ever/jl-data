...
function string{P}(x::BigFloat{P})
    ...
    lng = ccall((:mpfr_snprintf,:libmpfr), Int32, (Ptr{UInt8}, Culong, Ptr{UInt8}, Ptr{BigFloat{P}}...), buf, lng + 1, "%.Re", &x)
    if lng < k + 5 # print at least k decimal places
        lng = ccall((:mpfr_sprintf,:libmpfr), Int32, (Ptr{UInt8}, Ptr{UInt8}, Ptr{BigFloat{P}}...), buf, "%.$(k)Re", &x)
    elseif lng > k + 8
        buf = Array{UInt8}(lng + 1)
        lng = ccall((:mpfr_snprintf,:libmpfr), Int32, (Ptr{UInt8}, Culong, Ptr{UInt8}, Ptr{BigFloat{P}}...), buf, lng + 1, "%.Re", &x)
    end
...
print{P}(io::IO, b::BigFloat{P}) = print(io, string(b))
show{P}(io::IO, b::BigFloat{P}) = print(io, string(b))

print{P}(b::BigFloat{P}) = print(Base.STDOUT, string(b))
show{P}(b::BigFloat{P}) = print(Base.STDOUT, string(b))
