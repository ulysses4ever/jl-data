...
function updateT(M::Model)
    ...
    dims1  = ntuple(i->length(Endogenous(M.variables)[i].val),length(Endogenous(M.variables)))
    pdims1 = prod(dims1)
    Gdims = size(M.distribution.tG[1])

    # fill!(M.distribution.T,0.0)
    ...
    ne = length(Endo)
    for i = 1:N
        brackets = [findbracket(Pf[ie][i],Endo[ie].val.x) for ie = 1:length(Endo)]
        ...
        for S2 = 1:length(Stoc)
            if isag(Stoc[S2])
                w = vcat([w[:]*p for p ∈ Stoc[S2].val.T[:,S1[S2]]]...)
            else
                w = vcat([w[:]*p for p ∈ Stoc[S2].val.T[S1[S2],:]]...)
            end
        end
...
function updateT1(M::Model)
    ...
    for i = 1:N
        brackets = [findbracket(Pf[ie][i],Endo[ie].val.x) for ie = 1:length(Endo)]
        S1 = Int[findfirst(s.val.x,M.distribution.tG[findfirst(names(State(M.variables)),s.name)][i]) for s ∈ Stoc]
        ...
        for j = 1:length(abrackets)
            for jj ∈ CartesianRange(Sdim)
                ...
                for S2 = 1:length(Stoc)
                    if isag(Stoc[S2])
                        t*= Stoc[S2].val.T[jj.I[S2],S1[S2]]
                    else
                        t*= Stoc[S2].val.T[S1[S2],jj.I[S2]]
                    end
                end
...
function ∫(M::Model,V::Array{Float64})
    ...
    if all(![isag(s) for s in State(M.variables)])
        return sum(d)
    else
...
function updateAggregates(M::Model,ϕ=1.0)
    if any([isag(s) for s in State(M.variables)])
        agg=Aggregate(M.variables)
        for s ∈ Aggregate(M.variables)
            ag  = ∫(M,s.val.target[1],s.val.target[2])
            eag = filter(x->isag(x),State(M.variables))
            t = [vtype(e)==Endogenous ? -1 : 0 for e in eag]

            X   = ndgrid([e.val.x for e in eag]...)
            Xid = findfirst(State(M.variables,false),s)
            for ii = 1:length(X[1])
