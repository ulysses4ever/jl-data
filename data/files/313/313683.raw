...
function display_progress(itr::Int64, alpha::Float64, gamma::Float64, residuals::class_residuals, vars::class_variables, settings::class_settings)
	try
		output = @sprintf("%s %2.1e %2.1e %2.1e %2.1e %2.1e %2.1e %2.1e %2.1e %i\n", rpad(string(itr),3), alpha, gamma, vars.tau(), vars.kappa(), residuals.scaled_mu, residuals.r_G_norm_scaled, residuals.r_P_norm_scaled, residuals.r_D_norm_scaled, 0)
...
function homogeneous_algorithm(qp::class_quadratic_program, vars::class_variables, settings::class_settings)
	...
	try	
		...
		print_if("It | alpha | gamma | tau  | kappa  |  mu  |  gap  | primal | dual | trial#", settings.verbose)
