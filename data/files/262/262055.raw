...
# Those components are saved, and then b is filled with zero.
...
function threshold!(
    ...
    @inbounds for i = 1:n
        x[i] = ifelse(abs(x[i]) < tol, 0.0f0, x[i])
    end
    return nothing
end
...
function update_indices!(
    ...
    @inbounds @simd for i = 1:p
        idx[i] = ifelse(x[i] != 0.0f0, true, false)
    end
    return nothing
end
...
# This subroutine updates a submatrix Xk containing k columns of X as selected by an index vector idx.
# Compare this to
# > Xk = a*X[:,idx]
#
# Arguments:
...
# -- idx is a BitArray that indexes the columns of x to send into xk. It should have a true for each
...
function update_xb(
    ...
    Xb = SharedArray(Float32, n, init = S -> S[localindexes(S)] = 0.0f0, pids=procs())
    update_xb!(Xb, x, b, indices, k, p=p, n=n)
    return Xb
end
