const L			= generate_laplace_inner(m, n)/(dx^2)
const _L		= generate_laplace(m, n, dx)
...
@everywhere @inline function grad_slice!(grd_u_J, grd_v_J, I, p, u, v, Cx, Cy, LU, t)
	...
	# poissonproblem im Innern lösen. 0-rand kann einfach ignoriert werden
	pI_x			= pI_x[2:m-1, 2:n-1]
	pI_y			= pI_y[2:m-1, 2:n-1]

	phi_x			= solve_poisson(LU, pI_x)
	phi_y			= solve_poisson(LU, pI_y)

	# 0-Rand wieder dranschrauben
	phi_x			= [zeros(1,n) ; zeros(m-2) reshape(phi_x, m-2, n-2) zeros(m-2) ; zeros(1,n)]
	phi_y			= [zeros(1,n) ; zeros(m-2) reshape(phi_y, m-2, n-2) zeros(m-2) ; zeros(1,n)]
