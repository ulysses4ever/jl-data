...
immutable FastPoly{T<:Number}
    ...
    function FastPoly{S}(a::Vector{S}, var)
        # if a == [] we replace it with a = [0]
        if length(a) == 0
            return new{S}(zeros(S,1), Symbol(var))
        else
            # determine the last nonzero element and truncate a accordingly
            a_last = max(1,findlast(a))
            new{S}(a[1:a_last], Symbol(var))
        end
    end
end

FastPoly{S}(a::Vector{S}, var) = FastPoly{S}(a,var)
FastPoly{S}(a::Vector{S}) = FastPoly{S}(a,:x)
