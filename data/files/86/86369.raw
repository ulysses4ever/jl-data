...
function scriptwrite(filename,xml,dt,T,tau,outname,fffile,topfile,rcut,steps,therm,rbuff;restartname="restart.xml",bb1314=false,syms="allinputsym.txt",f1314="mapped.xml",dihtype="type9",restart=false,gamma=0,per=100,rseed=1234,solvent="W",harmcons=0,genvel=true)
...
#restartfile is set to true for writing a script that will restart from an xml file output by hoomd as a restart file.  If it is false, then we will write out an initialization file
#genvel is true if we want to generate velocities--will overwrite any added to the xml file
	f = open(filename,"w")
	println(f,"from hoomd_script import *")
	println(f,"from math import *")
	println(f,"import random")
	if restart!=false
		println(f,"from restarthoomd import *")
	end
	println(f,"context.initialize()")
	if restart!=false
		println(f,"system = init.read_xml(filename=\"$xml\", restart = \"$restartname\")")
	else
		println(f,"system = init.read_xml(filename=\"$xml\")")
	end
	println(f,"dumpname = '$outname'")
	if restart!=false
		println(f,"dropstep = getTimeStart(\"$restartname\")")
		println(f,"reLog(dumpname+'.log',dropstep)")
		println(f,"reDCD(dumpname+'.dcd',dropstep)")
	end
	...
	(beadlist,bondlist,anglist,dihlist) = readTop(topfile,dtype=dihtype)
	if bb1314
		bonds1314 = get1314(f1314,syms)
		b1314array = Array(Any,size(bonds1314,1),3)
		for k = 1:size(bonds1314,1)
			bline1314 = split(bonds1314[k])
			b1314array[k,:] = [bline1314[1] parse(Int,bline1314[2]) parse(Int,bline1314[3])]
		end
		bondlist = cat(1,bondlist,b1314array)
	end
	if solvent=="W"
		solvbead = "P4"
	end
	...
	println(f,"\tT = C1*sin(phi)+2.0*C2*sin(2.0*phi)+3.0*C3*sin(3.0*phi)+4*C4*sin(4.0*phi)+S1*sin(phi-pi/2.0)+2.0*S2*sin(2.0*phi-pi/2.0)+3.0*sin(3.0*phi-pi/2.0)+4.0*S4*sin(4.0*phi-pi/2.0)")
	...
	elseif dihtype == "type9plusmartini"
	...
	println(f,"\treturn (V,T)")
	println(f,"\n")
	println(f,"def type2(phi,k,phi0):")
	println(f,"\tV = 0.5*k*(phi-phi0)**2")
	println(f,"\tT = -k*(phi-phi0)")
	println(f,"\treturn (V,T)\n")
        for dih = 1:size(dihlist,1)
		if dihlist[dih,15] == "p"
			println(f,"dtable.dihedral_coeff.set('$(dihlist[dih,1])',func=type9, coeff=dict(C0=$(dihlist[dih,6]),C1=$(dihlist[dih,7]),C2=$(dihlist[dih,8]),C3=$(dihlist[dih,9]),C4=$(dihlist[dih,10]),S1=$(dihlist[dih,11]),S2=$(dihlist[dih,12]),S3=$(dihlist[dih,13]),S4=$(dihlist[dih,14])))")
		elseif dihlist[dih,15] == "i"
			println(f,"dtable.dihedral_coeff.set('$(dihlist[dih,1])',func=type2,coeff=dict(k=50.0,phi0=0.0))")			
		end
	end	
	elseif dihtype == "table" #dihedrals are the same as bonds/angles
		dihs = unique(dihlist[:,1])
		dfile = open("$(dihs[1]).dat","r")
		dlines = readlines(dfile)
		dtabwidth = length(dlines)
		println(f,"dtable = dihedral.table(width=$dtabwidth)")
		close(dfile)
		for dih in dihs
			println(f,"dtable.set_from_file('$dih','$dih.dat')")
		end
	else
		println("Unknown dihedral type.  Dihedrals may not be written correctly.")
	...
	println(f,"xml_restart = dump.xml(filename=\"$restartname\",all=True,restart=True,period=$(per*100),phase=0)")
	println(f,"analyze.log(filename=dumpname+'.log',quantities=['potential_energy','kinetic_energy','temperature','pressure'],period=$per,phase=0)")
	println(f,"dump.dcd(filename=dumpname+'.dcd',period=$per,unwrap_full=True,phase=0)")
	println(f,"run($steps)")
	close(f)
end
