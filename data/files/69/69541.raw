...
#In Wilkinson's fourth experiment, GFP is observed, but it is assumed to exactly reflect SigD concentration.
  obs_mol_name = "GFP"
...
#log uniform prior on 1*10^-4 to 1*10^0 for the 3 unknowns:
  ...
  #In Wilkinson's fourth experiment, GFP is observed, but it is assumed to exactly reflect SigD concentration.
  obs_mol_ind = Chem_rxn_tools.get_chem_indices(wilk_cri, "SigD")
...
using Dates
  metadata_to_save = string("This test was run at time ", now(), " with ",
                            ...
                            "The observed molecule was ", obs_mol_name,
                            ", but it was incorrectly taken to be SigD, which was at index ", Chem_rxn_tools.get_chem_indices(wilk_cri, obs_mol_name), ". \n")

today_filepath = string("/Users/EricKernfeld/Desktop/Spring_2015/518/eric_prelim_code/julia_version/project_specific/experiments_after_tidy/GFP_simplified_", now())
...
sim_results = Chem_rxn_tools.make_sim_data(t_obs, wilk_cri, obs_mol_name, noise_distribution)
  mols_to_show = ["SigD", "CodY", "Hag", "GFP"]
