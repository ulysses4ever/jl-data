#This file carries out Wilkinson's first experiment, observation of SigD.
...
#   molecule_index -- 1
function pMCMC_setup_experiment_3(;actually_run_MCMC=false)
  (true_init_x,
  sto_mat,
  rxn_entry_mat,
  true_rxn_rates,
  ...
  if(actually_run_MCMC==true)
    prior_sample_thetas = 10.^(-4*rand(num_rxns,num_samples_desired)) #log uniform prior on 1*10^-4 to 1*10^0
    prior_sample = Sample_state_and_params_type(prior_sample_thetas,
                        reshape(repmat(true_init_x, num_samples_desired),num_species,num_samples_desired))
...
true_rxn_rates, t_obs, d_obs, x_obs, x_path, true_init_x = pMCMC_setup_experiment_3(actually_run_MCMC=false)

are_we_profiling = false
tic()
#Actually run the sampler; update metadata_to_save with time taken
  Profile.clear()
  if(are_we_profiling)
    println("Not profiling.")
    posterior_sample,prior_sample, true_rxn_rates, t_obs, d_obs, x_obs, x_path, true_init_x, num_acc, metadata_to_save = @profile pMCMC_setup_experiment_3(actually_run_MCMC=true)
    ProfileView.view()
  else
    posterior_sample,prior_sample, true_rxn_rates, t_obs, d_obs, x_obs, x_path, true_init_x, num_acc, metadata_to_save = pMCMC_setup_experiment_3(actually_run_MCMC=true)
  end
  time_taken = toc()
...
today_filepath = string("/Users/EricKernfeld/Desktop/Spring_2015/518/eric_prelim_code/julia_version/experiment3",now())
...
#To save important metadata
  ...
  @save string(today_filepath, "/samples") posterior_sample prior_sample x_obs d_obs t_obs
