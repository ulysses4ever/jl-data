...
function cut_分词_words(code::String,engine::结巴分词,FILESMODE::Bool) 
    ...
    tempvector = ccall(vector_result_key,Ptr{Void},(Ptr{Void},Ptr{Uint8}),engine.固定元素.引擎,pointer(code))
    elseif engine.固定元素.引擎类型 == "概率"
    tempvector = ccall(mp_vector_result_key,Ptr{Void},(Ptr{Void},Ptr{Uint8}),engine.固定元素.引擎,pointer(code))
    elseif engine.固定元素.引擎类型 == "hmm"
    tempvector = ccall(hmm_vector_result_key,Ptr{Void},(Ptr{Void},Ptr{Uint8}),engine.固定元素.引擎,pointer(code))
    elseif engine.固定元素.引擎类型 == "索引"
    tempvector = ccall(qu_vector_result_key,Ptr{Void},(Ptr{Void},Ptr{Uint8}),engine.固定元素.引擎,pointer(code)) 
    end
