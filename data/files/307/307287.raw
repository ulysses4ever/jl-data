...
function SeisPlot(in,param=Dict())
    ...
    vmin = get(param,"vmin",-0.95*maximum(abs(in[:])))
    vmax = get(param,"vmax",0.95*maximum(abs(in[:])))
	...
	for k = 1:wiggle_trace_increment:size(in,2)
 	    ...
 	    sp = (s+abs(s))/2
            ...
            if (canvas != "NULL")
                # draws the wiggle line of current trace
                wigline = lines.Line2D(s*alpha + x[k], y, color=wiggle_line_color)
                canvas[:add_line](wigline)
                # draws the vertical line through the current trace
                for i = 1:1:size(in,1)
                    push!(x_vert, k)
                    push!(y_vert, i)
                end
