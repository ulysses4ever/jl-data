...
while difftrans>difftranstol
    ...
    println("**********", iteration, "-th iteration **********")
    tic()
    ...
    while diffmax>0        # stop when the policies converge.
        print("Value function iteration starts ")
        print(".")
        ...
        for i=1:npgridsize
            for j=1:zgridsize
                for k=1:xigridsize
                    ...
                    Vnoadjustnew[i,j,k]=( exp(        np[i,1]  )-omega/exp(z[ j ,1 ] ) )*(exp( (a0 +a1*xi[ k ,1 ] )*(theta-2) )*exp(-theta*np[i ,1 ]  ));
                    ...
                    for n=1:zgridsize # n for integration
                        ...
                        for r=1:moneygridsize
                            ...
                            if 1<=i-(1+(r-(moneygridsize+1)/2)) && npgridsize>=i-(1+(r-(moneygridsize+1)/2)) # when log(p/S) is in the bounds
                                ...
                                Vnoadjustnew[ i , j , k ]=Vnoadjustnew[ i , j , k ]+Probmoney[r,1] * beta * exp(    -(a0+a1*xi [ k,1 ]  ) )/exp(-(a0+a1* xi[ Xiprime [ k , r ],1  ]    ))* Probz[ j , n ]*max(  Vnoadjustold[   i-1-(r-(moneygridsize+1)/2 ) , n , Xiprime[k,r]  ],  Vadjustold[n,Xiprime[k,r] ]   )
                            ...
                            elseif 1>i-(1+(r-(moneygridsize+1)/2)) # when log(p/S') is out of the grid (too small)
                                ...
                                Vnoadjustnew[i,j,k]=Vnoadjustnew[i,j,k] + Probmoney[r,1]*  beta*   exp(-(a0+a1*   xi[k,1]   ))/exp(-(a0+a1*      xi[Xiprime[k,r],1  ]      ))*Probz[j,n]*    max(  Vnoadjustold[ 1 , n,Xiprime[k,r]  ],  Vadjustold[n,Xiprime[k,r]]   );

                            else # when log(p/S) is too big

                                # Vold=max(  Vnoadjustold(npgridsize ,  n,Xiprime(k,r)),  Vadjustold(n,Xiprime(k,r))    );

                                Vnoadjustnew[i,j,k]= Vnoadjustnew[i,j,k] + Probmoney[r,1]*beta*exp(-(a0+a1*  xi[k]  ))/exp(-(a0+a1*         xi[Xiprime[k,r] ,1 ]    ))*Probz[j,n]*max(  Vnoadjustold[npgridsize ,  n,Xiprime[k,r]],  Vadjustold[n,Xiprime[k,r]]    );
                            end
        ...
        for j=1:zgridsize
            for k=1:xigridsize
                ...
                Vadjustnew[j,k]=Vadjustnew[j,k]-fmenu*omega*exp( -(a0+a1*xi[k,1]  ) );
            end
        end
...
for k=1:numsim+burnin
    ...
    if moneyrand<Probmoneycum[1,1] # moneyshock is 1-st grid
        moneyholder=1+(1-(moneygridsize+1)/2);
        moneyshockindex=1;
    else

        for j=1:moneygridsize-1
            if moneyrand>Probmoneycum [ j ,1 ] && moneyrand <= Probmoneycum[ j+1,1 ] # money shock is j+1
    ...
    for i=1:numfirms
        ...
        if k>burnin  #Calculate statistics:
            if pricetoday[i,1]>currentfirmstate[i,1] #<=> pricetoday(i,1)>price yesterday(i,1)
                ...
                size2=size2+np[pricetoday[i,1] ,1 ]-np[currentfirmstate[i,1],1  ];
            elseif pricetoday[i,1]<currentfirmstate[i,1]
                numdown=numdown+1;
                numdown2=numdown2+1;
                sizedown=sizedown+np[currentfirmstate[i,1] ,1  ]-np[pricetoday[i,1],1  ];
                sizedown2=sizedown2+np[currentfirmstate[i,1] ,1 ]-np[pricetoday[i,1] ,1];
                size2=size2+np[pricetoday[i,1] ,1 ]-np[currentfirmstate[i,1] ,1 ];
            end
        end
    ...
    #-------------------------------------------------------------------------------
    if k>burnin
        xsd=xsd+sum(            (        ( np[ pricetoday[:,1],1 ]  -np[ currentfirmstate[:,1],1 ] )     -    mean(    np[    pricetoday[:,1] ,1   ]  -    np[    currentfirmstate[:,1] ,1  ]  )         ).^2/numfirms)^.5;
        xsdholder[k-burnin]=sum(       (      (  np[pricetoday[:,1] ,1 ]-np[currentfirmstate[:,1] ,1 ]   )-mean (    np[pricetoday[:,1] ,1  ] - np[currentfirmstate[:,1] ,1  ]    )).^2/numfirms)^.5;
        meanpricechange[k-burnin]=mean(        np[pricetoday[:,1] ,1  ]-np[currentfirmstate[:,1] ,1  ]   );
    end
    ...
    if k>burnin
        ...
        xisim[k-burnin]=log((sum((exp(         np[currentfirmstate[:,1],1]   )).^(1-theta))/numfirms)^(1/(1-theta)));
...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    println(" ")
    println("Law of capitl (old) - Law of capita(new) : " , difftrans)
    # Update the coefficients for aggregate state
    a0=a0new;
    a1=a1new;
    t=toq()
    println(iteration,"-th outer loop takes ",round(t)," sec" )
    println(" ")
end
