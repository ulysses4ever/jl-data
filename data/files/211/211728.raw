...
# supplemented by Yuta Takahashi
...
#np=nan(npgridsize,1);
for i=1:npgridsize
    np[i]=npmin+(npmax-npmin)/(npgridsize-1)*(i-1);
end
#z=nan(zgridsize,1);
for i=1:zgridsize
    z[i]=zmin+(zmax-zmin)/(zgridsize-1)*(i-1);
end

#xi=nan(xigridsize,1);
for i=1:xigridsize
    xi[i]=ximin+(ximax-ximin)/(xigridsize-1)*(i-1);
end

#money=nan(moneygridsize,1);
for i=1:moneygridsize
    money[i]=moneymin+(moneymax-moneymin)/(moneygridsize-1)*(i-1);
end
...
Probz=zeros(zgridsize);
for j=1:zgridsize
    Probz[j,1]=normcdf((z[1]-rho_z*z[j]+w/2)/sigma_z,0,1);
    Probz[j,zgridsize]=1-normcdf((z(zgridsize)-rho_z*z[ j ]-w/2)/sigma_z,0,1);
    for k=2:zgridsize-1
        Probz[j,k]=normcdf((z [ k ]-rho_z* z [ j ]+w/2)/sigma_z,0,1)-normcdf((z[k]-rho_z*z[j]-w/2)/sigma_z,0,1);
    end
end

#Calculate CDF:
Probzcum=zeros(zgridsize,1);
for j=1:zgridsize
    for k=1:zgridsize
...
Probmoney[1]=normcdf((money[1]+w/2)/sigma_eps,0,1);
Probmoney(moneygridsize)=1-normcdf((money[moneygridsize]-w/2)/sigma_eps,0,1);

for j=2:moneygridsize-1
    Probmoney[j]=normcdf((money[j]+w/2)/sigma_eps,0,1)-normcdf((money[j]-w/2)/sigma_eps,0,1);
end
Probmoneycum=zeros(moneygridsize,1);
for j=1:moneygridsize
    Probmoneycum[j]=sum(Probmoney[1:j]);
end
