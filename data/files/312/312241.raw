...
rands = "$rands"
# `search("[") + 1` skips to the first character after the type indicator at
#       the beginning of the String.
# `search("]") - 1` drops the final `]`.
word_indexes = [strip(x) for x in split(rands[search(rands, '[') + 1 :search(rands, ']') - 1], "\n")]
# word_indexes = arrayToString(rands, 5)
for w in word_indexes
    passphrase = passphrase * wordlist[w] *  " "
end
split(passphrase)
end
