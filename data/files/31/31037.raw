...
`deg`, `min`, and `sec` may be either all scalars or all vectors of scalars.

The string should have the form `"deg:min:sec"` or `"deg min sec"`.  Also an
array of strings is accepted.

`tenv` is an alias for `ten`.

**NOTE:** This function cannot deal with `-0` (negative integer zero) in numeric
input.  If it is important to give sense to negative zero, you can either make
