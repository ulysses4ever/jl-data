...
function addcounts!(h::Histogram, events::Vector)
...
Add events to `h` whose values are in `events`. Only add events for which `selection` (a bit vector or Vector{Bool}) is true. Add
events for which `selection` is false to to `h.seen`."""
function update_histogram!(h::Histogram, selection, events)
	addcounts!(h,events[selection])
...
function (-)(a::Histogram, b::Histogram)
	@assert edges(a)==edges(b) "subtracting histograms only supported if the x is the same, a has $(edges(a)), b has $(edges(b))"
	c = Histogram(edges(a), a.counts - b.counts, a.seen - b.seen, a.counted - b.counted)
end
