...
const TIMEWINDOWS = [(0.0, 20.0), # D
                     (0.0, 20.0), # N1
                     (19.0, 20.0), # N2 
                     (19.0, 20.0), # N3 (can't do N2 and N3 in one run)
                     (0.0, 20.0)] # N4
const CAPACITY = 80.0
const MAXVEHICLES = 4
...
function SolveMaster(routes)
  ...
  # Build array of selected routes
  selected_routes = Any[]
  for r = 1:num_routes
    if getValue(x[r]) > 0.9
      push!(selected_routes, routes[r][1])
    end
  end
  ...
  println("  MASTER: Solved, returning primal/dual")

  return selected_routes, dual
end
...
function SolveSubproblem(dual)
  ...
  for i = 0:NODES
    for j = 1:NODES # Don't worry about arcs into depot just yet
      @addConstraint(m, A[i] + NODEDISTS[i+1,j+1] <= A[j] + BIGM*(1.0 - x[i,j]))
    end
  end
...
for iter = 1:10
  println("ITER $iter")
  selected_routes, dual = SolveMaster(routes)
