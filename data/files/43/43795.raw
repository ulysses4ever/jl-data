#=
 ...
 = (nonextrapolated) from the various in situ and analysis subdirs - RD April 2016.
 =#
...
if contains(ARGS[1], "lhfx")
  dirs = [        "erainterim", "hoaps", "ifremerflux", "jofuro", "merra", "oaflux", "seaflux"] ; fend = ".noml"
elseif contains(ARGS[1], "airt") || contains(ARGS[1], "sstt")
  dirs = ["cfsr", "erainterim", "hoaps", "ifremerflux",           "merra", "oaflux", "seaflux"] ; fend = ".nomt"
else
  dirs = ["cfsr", "erainterim", "hoaps", "ifremerflux", "jofuro", "merra", "oaflux", "seaflux"] ; fend = ".nomb"
end
(dirn,) = size(dirs)
...
for line in eachline(fpa)                                                     # loop through the insitu locations
  ...
  now = fill(MISS, dirn)                                                      # collate analysis and insitu data
  flag = true
  for (a, dira) in enumerate(dirs)
    fpc = My.ouvre("$dira/$dira.$tail", "r", false) ; linb = readlines(fpc) ; close(fpc) ; fpc = 0

    tmp = split(linb[datind]) ; now[a] = float(tmp[vind])
                                newdat = tmp[4][1:8] ; if dat != newdat  println("ERROR : $dat != newdat")  end
    if now[a] < -333.0 || now[a] > 3333.0  flag = false  end
  end
  gc()

  if flag                                                                     # and store the line if all values exist
    for (a, dira) in enumerate(dirs)
      tmp = @sprintf(" %9.3f", now[a]) ; out *= tmp
    end
    out *= "\n" ; write(fpb, out)
  end
end
