...
if size(ARGS) != (2,)
  write("\nUsage: jj $(basename(@__FILE__)) all.flux.locate.min2000 cfsr\n\n")
  exit(1)
end
...
for a = 1:locs
  if csv[a,3] > 0
    ...
    tmpd = "$(ARGS[2])/$(ARGS[2]).$tmpb.fftt"
    tmpe = "$(ARGS[2])/$(ARGS[2]).$tmpb.ffttest"
...
tmpa = @sprintf("spectrun.%s.%s", ARGS[1], ARGS[2])                           # store the weighted average spectrum
fpa = My.ouvre(tmpa, "w")
