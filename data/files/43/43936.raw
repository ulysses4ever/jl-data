...
tmp = Table(3,1)
tmp[1,1] = Winston.imagesc(shfx)
setattr(tmp[1,1].x2, "ticks",                  xpos) ; setattr(tmp[1,1].x1, "draw_nothing",           true)
setattr(tmp[1,1].x2, "tickdir",                   1) ; setattr(tmp[1,1].x1, "tickdir",                   1)
setattr(tmp[1,1].x2, "ticklabels",             xlab) ; setattr(tmp[1,1].x1, "draw_ticks",            false)
setattr(tmp[1,1].x2, "draw_subticks",         false) ; setattr(tmp[1,1].x1, "draw_subticks",         false)
#setattr(tmp[1,1].x1, "ticklabels_style.rotation", "90")
setattr(tmp[1,1].y1, "ticks", collect(100:200:1700))#; setattr(tmp[1,1].y2, "ticks", collect(100:200:1700))
setattr(tmp[1,1].y1, "tickdir",                   1) ; setattr(tmp[1,1].y2, "tickdir",                   1)
setattr(tmp[1,1].y1, "ticklabels",             dirs) ; setattr(tmp[1,1].y2, "draw_ticks",            false)
setattr(tmp[1,1].y1, "draw_subticks",         false) ; setattr(tmp[1,1].y2, "draw_subticks",         false)
ppp = Winston.add(tmp[1,1])
