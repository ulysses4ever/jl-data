...
function init_channel(h5::Union(HDF5File, HDF5Group), ljhgroup::LJHGroup)
	...
	a_require(g, "pulsefile_lengths", lengths(ljhgroup))
	a_require(g, "npulses", length(ljhgroup))
	a_require(g, "record_nsamples", record_nsamples(ljhgroup))
	a_require(g, "pretrig_nsamples", pretrig_nsamples(ljhgroup))
	a_require(g, "timebase", frametime(ljhgroup))
	a_require(g, "channel", channel(ljhgroup))
	gitstate = Pkg.Git.snapshot(dir=Pkg.dir("Mass"))
	a_require(g, "git_state", [getfield(gitstate, s) for s in names(gitstate)])
	a_require(g, "mass_version", string(Pkg.installed("Mass")))
	a_require(g, "julia_version", versioninfostr())
	g
end
...
function summarize(r::UnitRange,pulsefile_names,pulsefile_lengths)
	...
	tuple(new_lengths, [getfield(pulse_summaries, n) for n in names(pulse_summaries)]...)
end
...
compute_summary(ljh, 1:1);
compute_summary(ljh.ljhfiles[1], 1:1);
println("timing comparisons")
println("computer_summary")
@time compute_summary(ljh, 1:50000);
println("compute_summary(::LJHFile)")
@time compute_summary(ljh.ljhfiles[1], 1:50000);
