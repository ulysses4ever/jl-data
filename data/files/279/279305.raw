#TODO: get rid of the camelCase arguments
"""
...
* `bounds::Matrix{T}=Array(T,0,0)`: clamp solution to the range in this n x 2 matrix
"""
function levenberg_marquardt{T}(f::Function, g::Function, x0::AbstractVector{T};
    ...
    lambda::Real = 10.0, show_trace::Bool = false, bounds::Matrix{T} = Array(T,0,0))
    ...
    while (~converged && iterCt < maxIter)
        ...
        if rho > MIN_STEP_QUALITY
            x += delta_x
            isempty(bounds) || (x = map(i->clamp(x[i],bounds[i,:]...), 1:length(x)))
            fcur = trial_f
