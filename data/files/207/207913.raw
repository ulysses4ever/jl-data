...
function loadings(lc::LC,
    ...
    load[L_INIT] = products[lc.all[i,"prod_id"],l_type * "_C_INIT"]
    load[L_ABS]  = products[lc.all[i,"prod_id"],l_type * "_C_ABS"]
    load[L_IS]   = products[lc.all[i,"prod_id"],l_type * "_C_IS"]
    load[L_PREM] = products[lc.all[i,"prod_id"],l_type * "_C_PREM"]
    return load
end


function interpolate(c1::Int, c2::Int, v1::Number, v2::Number)
    v1 + [0:(c2-c1)] / max(1, c2-c1) * (v2-v1)
end
...
function profile( lc::LC,
    ...
    for WX in keys(d)
        if ((lc.all[i,"c_start_"*WX] > 0) &
            (lc.all[i,"c_end_"*WX] > 0))
            prof[lc.all[i,"c_start_"*WX]:lc.all[i,"c_end_"*WX],
                 d[WX]] =
                     interpolate(lc.all[i,"c_start_"*WX],
                                 lc.all[i,"c_end_"*WX],
...
function sx ( lc::LC, i::Int, products::DataFrame )
    ...
    if !((lc.all[i,"c_start_SX"]==0) | (lc.all[i,"c_end_SX"]==0))
        prob_sx[lc.all[i,"c_start_SX"]:lc.all[i,"c_end_SX"] ] =
            interpolate(lc.all[i,"c_start_SX"],
                        lc.all[i,"c_end_SX"],
                        products[lc.all[i,"prod_id"],"start_SX"],
...
function price(lc::LC,
    ...
    unshift!(lx_bop,1)
    pop!(lx_bop)
    v = cumprod(exp(-convert(Array, tech_interest[1:lc.all[i,"dur"],
                                                  products[lc.all[i,"prod_id"],
                                                           "interest_name"]]) ) )
    v_bop = deepcopy(v)
    unshift!(v_bop,1)
...
function lc!(lc::LC,
    ...
    lc.n = size(lc.all,1)    
    lc.all["prod_id"] = [prod_id_dict[lc.all["prod_name"][i]] for i = 1:lc.n]
    lc.all["ph_age_start"] =  lc.all["y_start"] - lc.all["ph_y_birth"]
    lc.all["qx_name"] =
        [ ascii( lc.all[i,"ph_gender"] * "_" *
                products[prod_id_dict[lc.all["prod_name"][i]],"qx_name"])
            for i = 1:lc.n]
