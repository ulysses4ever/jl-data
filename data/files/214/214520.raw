#Kepler's Law
@everywhere semimajor_axis(P::Real, M::Real, G::Real=4pi*pi) = (G*M/(4pi*pi)*(P/days_in_year)^2)^(1.0/3.0)
@everywhere @vectorize_2arg Real semimajor_axis
...
# Calculates a "distance" between observations and draw(s) from a model 
# Optionally calculates an average distance using num_evals different realizations from same model
function evaluate_model_par(stats_obs, eta::Real, shape::Real, scale::Real, num_stars::Integer; minP=0.0, maxP = Inf, num_evals::Integer = 1 )
  @assert(0.0<=eta<=1.0)
  @assert(num_stars>0)
  @assert(minP>=0.0)
  @assert(maxP>minP)
...
function eval_model_on_grid_loops_par(etas::Array, shapes::Array, scales::Array, num_stars = 1600; num_evals = 1, 
  ...
  for k in 1:length(scales)
    for j in 1:length(shapes)
      for i in 1:length(etas)
        dist[i,j,k] = evaluate_model_par(stats_obs, etas[i], shapes[j], scales[k], num_stars; minP=minP, maxP=maxP, num_evals=num_evals )
      end # for i
    end # for j
  end # for k
  dist
...
function eval_model_on_grid_map(etas::Array, shapes::Array, scales::Array, num_stars = 1600; num_evals = 1, 
   ...
   # idx = [ (i,j,k) for i in etas, j in shapes, k in scales]
   # dist = map(tuple->evaluate_model_par(stats_obs, tuple[1], tuple[2], tuple[3], num_stars; minP=minP, maxP=maxP, num_evals=num_evals),idx)
   # dist = reshape(dist,4,4,4)
   # dist

end # function

function eval_model_on_grid_pmap(etas::Array, shapes::Array, scales::Array, num_stars = 1600; num_evals = 1, 
                            true_eta = 0.2, true_shape = 0.1, true_scale = 1.0)
    const solar_radius_in_AU = 0.00464913034
    minP = (2.0*solar_radius_in_AU)^1.5
    maxP = 4*days_in_year/3
    ...
    idx = [ (i,j,k) for i in 1:length(etas), j in 1:length(shapes), k in 1:length(scales) ]
    #We are not working with distributed arrays - then OK to use pmap -> discussion in class
    pmap(tuple -> evaluate_model(stats_obs, etas[tuple[1]], shapes[tuple[2]], scales[tuple[3]], num_stars; minP=minP, maxP=maxP, num_evals=num_evals ), idx)
...
function eval_model_on_grid_map_distributed(etas::Array, shapes::Array, scales::Array, num_stars = 1600; num_evals = 1,  true_eta = 0.2, true_shape = 0.1, true_scale = 1.0)
    ...
    #We are now working with distributed arrays -> discussion in class: we should use map, rather
    #than pmap
    map(tuple -> evaluate_model(stats_obs, etas[tuple[1]], shapes[tuple[2]], scales[tuple[3]], num_stars; minP=minP, maxP=maxP, num_evals=num_evals ), idx)
