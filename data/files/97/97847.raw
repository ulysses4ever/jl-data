...
function Earth_LatLong_Coord(latitude::Degree,longitude::Degree,bearing::String)
	latnumber::Float64 = degree_to_decimal_degree(latitude)
	longnumber::Float64 = degree_to_decimal_degree(longitude)
...
function sign_lat_long(latlong::LatLong_Coord)
	...
	(newlat::Float64,newlong::Float64) = sign_lat_long(latlong.latitude,latlong.longitude,latlong.bearing)
end
...
function convert(Spherical_Coord,latlong::LatLong_Coord)
	...
	phi::Float64 = to_radians(signedlat)
	theta::Float64 =  to_radians(signedlong)
...
function to_radians(degree::Real)
	...
	_sign = sign(degree)
	reduced_degree::Float64 = mod(abs(degree),360)
	radians::Float64 = reduced_degree*(pi/180)
...
function get_distance(c1::Spherical_Coord,c2::Spherical_Coord)
	...
	central_angle::Float64 = vincenty(c1,c2)
	#=central_angle = haversine(c1,c2)=#
	#=central_angle = alternative_haversine(c1,c2)=#
	distance::Float64 = c1.radius*central_angle
...
function get_distance(c1::Sphere_LatLong_Coord,c2::Sphere_LatLong_Coord)
	...
	return get_distance(c1s,c2s)::Float64
end
...
function vincenty(c1::Spherical_Coord,c2::Spherical_Coord)
	...
	longdiff::Float64 = c1.theta-c2.theta
	...
	vin_numerator::Float64 = ( (cos(c2.phi)*sin(longdiff))^2 + ( cos(c1.phi)*sin(c2.phi) - sin(c1.phi)*cos(c2.phi)*cos(longdiff))^2 )^(1/2) 
	vin_denominator::Float64 = sin(c1.phi)*sin(c2.phi)+cos(c1.phi)*cos(c2.phi)*cos(longdiff)
	central_angle::Float64 = atan2(vin_numerator,vin_denominator)
...
function spherical_law_of_cosines(c1::Spherical_Coord,c2::Spherical_Coord)
	...
	longdiff::Float64 = abs(c1.theta-c2.theta)
	central_angle::Float64 = acos(sin(c1.phi)*sin(c2.phi)+cos(c1.phi)*cos(c2.phi)*cos(longdiff))
	return central_angle
end
function haversine(c1::Spherical_Coord,c2::Spherical_Coord)
	longdiff::Float64 = c1.theta-c2.theta
	latdiff::Float64 = c1.phi-c2.phi
	a::Float64 = (sin(latdiff/2))^2 + cos(c1.phi)*cos(c2.phi)*(sin(longdiff/2))^2
	central_angle::Float64 = 2*atan2(sqrt(a),sqrt(1-a))
	return central_angle
end
function alternative_haversine(c1::Spherical_Coord,c2::Spherical_Coord)
	...
	latdiff::Float64 = abs(c1.phi-c2.phi)
	longdiff::Float64 = abs(c1.theta-c2.theta)
	argument::Float64 = sqrt(sin((latdiff)/2)*sin((latdiff)/2) - cos(c1.phi)*cos(c2.phi)*sin((longdiff)/2)*sin((longdiff)/2))
	central_angle::Float64 = 2*asin(argument)
	return central_angle
end
