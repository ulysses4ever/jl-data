...
mymh_a = MPM.MHRecord(obj_a,burn=5000,thin=50)
sample(mymh_a,10000)
...
mymh_b = MPM.MHRecord(obj_b,burn=5000,thin=50)
sample(mymh_b,10000)
...
ga,lams = MPM.calc_g(grid, mymh_a.db, 20)
...
figure()
gb,lams = MPM.calc_g(grid, mymh_b.db, 20)
...
println("$(MPM.error_points(mymh_a.db, mymh_b.db, [tst_data; tst_datab], [zeros(size(tst_data,1)), ones(size(tst_datab,2))] ))")
...
##### TRUE #####
#truept = MPM.MPMParams(trumu, #mu :: Matrix{Float64}
...
#figure()
#gavg,lams = MPM.calc_g(grid, [start], 500)
#imshow(reshape(gavg,N,N)', extent=gext, aspect="equal", origin="lower")
#colorbar()
#contour(reshape(gavg,N,N)', extent=gext, aspect="equal", origin="lower")
#plot(data[:,1], data[:,2], "g.", alpha=0.8)
