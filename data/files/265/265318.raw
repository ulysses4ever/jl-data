...
type QRProblem
  ...
  up        # n variables epsilon positive
  un        # n variables epsilon negative
  xi_dual   #
  n::Int64
  p::Int64
  oneN::Array{Float64, 1}

  function QRProblem(solver::AbstractMathProgSolver, X::Array{Float64, 2}, Y::Array{Float64, 1}, T::Array{Float64,1},t0::Float64)
    n, p = size(X)
    h=n^(-1/3)
## for loop
    stdX = zeros(n)
    ...
    @JuMP.addConstraint(problem, xi_dual[i=1:n], Y[i] - intercept - dot(vec(X[i,:]), beta) == up[i] - un[i]) ## epsilon
    for i=1:n
