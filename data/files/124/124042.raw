...
function mistakecounts_20news(num::Int)
    ...
    ftotal = open("./results-20news/avg/total-$(lpad(num, 2, 0)).csv", "w")
    fpercent = open("./results-20news/avg/percent-$(lpad(num, 2, 0)).csv", "w")
...
function mistakecounts_reviews(num::Int)
    ...
    ftotal = open("./results-reviews/avg/total-$(lpad(num, 2, 0)).csv", "w")
    fpercent = open("./results-reviews/avg/percent-$(lpad(num, 2, 0)).csv", "w")
    for i = 1:length(total)
        write(ftotal, "$(i * step), $(join(total[i], ", "))\n")
        write(fpercent, "$(i * step), $(join(percent[i], ", "))\n")
    end
