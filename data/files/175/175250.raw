...
function sampletopicindex(nk, nwz, w1, w2, W, K, tau, alpha, beta, delta)
    ...
    p[1] = (nk[1] + tau[1] * alpha) * (max(nwz[w1, 1] - delta, 0) + beta) * (max(nwz[w2, 1] - delta, 0) + beta) / ((2 * nk[1] + W * beta) * (2 * nk[1] + (W + 1) * beta))
	  for k = 2:K 
		    p[k] = p[k-1] + (nk[k] + tau[k] * alpha) * (max(nwz[w1, k] - delta, 0) + beta) * (max(nwz[w2, k] - delta, 0) + beta) / ((2 * nk[k] + W * beta) * (2 * nk[k] + (W + 1) * beta))
	  end
...
(topics, nk, nwz, K, Kmax, U1, U0, tau) = initialize(dictionary, biterms, length(dictionary), 1000, parsed_args["alpha"], parsed_args["beta"], parsed_args["gamma"], parsed_args["delta"])
(topics, nk, nwz, K, Kmax, U1, U0, tau) = gibbs(biterms, topics, nk, nwz, length(dictionary), K, Kmax, U1, U0, tau, parsed_args["alpha"], parsed_args["beta"], parsed_args["gamma"], parsed_args["delta"], parsed_args["iterations"])
