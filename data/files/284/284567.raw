...
facts ("Pruebas-Intervalos") do
  ...
  @fact 10*M==M*10=>true
  @fact M/N=>Interval(0.14285714285714285, 1.6666666666666667)
  @fact M+1.0=>Interval(2.0, 6.0)
  @fact M*Interval(2.0)=>Interval(2.0,10.0)
  ...
  @fact contains(exp(M),exp(M).left)=>true
  @fact contains(exp(M),exp(M).right)=>true
  @fact exp(log(y))==y => true
  @fact length(M)==length(N)=>true
  @fact Intersection(P,y)==Interval(0.0,1.0)=>true
  @fact Midpoint(P)==0.0=>true
