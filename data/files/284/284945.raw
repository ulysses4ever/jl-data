...
function find_interior_last!(cost::CostParameters, mp::RLSModel, ic1, ic2, ic, c, τ, ESS)
    ...
    a = r1(cost.C[ic1], cost.C[ic2]) - K(c, cost) + mp.β*H1(ic, ic2, pi, τ, ESS)
    b = mp.β*(H1(ic, ic, pi, τ, ESS)-H1(ic, ic2, pi, τ, ESS))
    ...
    A = r2(cost.C[ic1], cost.C[ic2]) - K(c, cost) + mp.β*H2(ic1, ic, pi, τ, ESS)
    B = mp.β*(H2(ic, ic, pi, τ, ESS)-H2(ic1, ic, pi, τ, ESS))
        ...
        for j = 1:length(pstar2)
            if i in 1:2 && j in 1:2
                ...
                if abs(exP1 - pstar1[i]) < 1e-8 && abs(exP2-pstar2[j]) < 1e-8
                    ...
                    @inbounds τ[end].EQs[ic1, ic2, count] = EQ(pstar1[i],
