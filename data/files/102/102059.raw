...
# add merit function/line search
...
function main() # this function is called at the bottom of the code
	...
	settings.bkscale = 0.9;  # Back scaling for line search.
...
function netlib_batch_test(settings)
	list_of_filenames = ["Problems/BNL2.mat"]
	
	for filename in list_of_filenames
		problem_data = construct_instance_from_mat(filename);
		#problem_data.add_inequality_constraint(problem_data.c',-30.84,1.0)
		solve_with_JuMP(problem_data,IpoptSolver(max_iter=100))
		solve_with_JuMP(problem_data,MosekSolver())
...
function solve_with_JuMP(problem_data::class_linear_program_input,solver=IpoptSolver())
	...
	@setObjective( model, Min, sum{ c[j]*x[j] , j=1:k} ) # + -2*0.5*x[j]*x[j] 
	# 0.000001*(-(x[j]-5.0)^2)
...
function construct_instance2() # example synced with c++
	...
	problem_data.k = k
	problem_data.n = n
	problem_data.barrier_weights = ones(4);
...
function add_nonlinear_constraints(problem_data)
	...
	evaluate_inequality_constraint = function(x)
		...
		return [problem_data.G[1:(problem_data.m-1),:]*x; x'*x] - [_h[1:problem_data.m-1]; 0];  #  - 2*0.0001*x
