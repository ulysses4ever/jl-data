...
function getNeighborhoodD!(
  ...
  # process node
  for k=1:K
    out[k, 1] = phi_node_d(X[rowInd, nodeInd], k)
  end
  ...
  for b=1:p
    ...
    for l=1:L
      _ci = K + (indEdge-1)*L + l
      out[_ci, 1] = phi_edge_d(X[rowInd, nodeInd], X[rowInd, b], l, 1)
    end

    # derivative with respect to second argument
    for l=1:L
      _ci = K + (indEdge-1)*L + l
      out[_ci, 2] = phi_edge_d(X[rowInd, nodeInd], X[rowInd, b], l, 2)
    end
...
function getNeighborhoodE(
    X::Array{Float64, 2}, nodeInd::Int64, K::Int64, L::Int64,
    phi_node_d_2::Function, phi_edge_d_2::Function)
  ...
  for rowInd=1:n
    # process node
    for k=1:K
      E[k] += phi_node_d_2(X[rowInd, nodeInd], k)
    end
    ...
    for b=1:p
      ...
      numEdges += 1
      for l=1:L
        _ci = K + (numEdges-1)*L + l
        E[_ci] += phi_edge_d_2(X[rowInd, nodeInd], X[rowInd, b], l, 1)
                  + phi_edge_d_2(X[rowInd, nodeInd], X[rowInd, b], l, 2)
      end
    end
  end
  E
