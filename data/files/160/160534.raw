function calc_wignerfunction_resonator(cutoffN,wignerSamples,hamiltonian,coupling_freq,initialstate,time_vec,time_evo_array)
  ...
  for realIndex=1:wignerSamples
    for imagIndex=1:wignerSamples
      gen_displacementoperator = kron(eye(2),gen_displacementop((realIndex-wignerSamples/2)/10 + 1im*(imagIndex-wignerSamples/2)/10,cutoffN)) # generating the displacement operator for angle m
      # For each displaced state, we first find the photon number distribution
      excited_prob = calc_qubittimeevo(gen_displacementoperator'*initialstate*gen_displacementoperator,time_evo_array)
