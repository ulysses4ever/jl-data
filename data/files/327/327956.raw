...
## reinterpret: Color->T
# Arrays
reinterpret{CV1<:Color,CV2<:Color}(::Type{CV1}, A::Array{CV2,1}) = _reinterpret_cvarray(CV1, A)
reinterpret{CV1<:Color,CV2<:Color}(::Type{CV1}, A::Array{CV2})   = _reinterpret_cvarray(CV1, A)
reinterpret{T,CV<:Color}(::Type{T}, A::Array{CV,1}) = _reinterpret_cvarray(T, A)
reinterpret{T,CV<:Color}(::Type{T}, A::Array{CV})   = _reinterpret_cvarray(T, A)
reinterpret{T,CV<:Color}(::Type{T}, A::StridedArray{CV})   = slice(_reinterpret_cvarray(T, A.parent), A.indexes...)
function _reinterpret_cvarray{T,CV<:Color}(::Type{T}, A::Array{CV})
...
reinterpret{CV<:Color}(A::StridedArray{CV}) = reinterpret(eltype(CV), A)

# Images
reinterpret{CV1<:Color,CV2<:Color}(::Type{CV1}, img::AbstractImageDirect{CV2}) =
    shareproperties(img, reinterpret(CV1, data(img)))
function reinterpret{CV2<:Color}(::Type{Uint32}, img::AbstractImageDirect{CV2})
...
function reinterpret{T,CV2<:Color}(::Type{T}, img::AbstractImageDirect{CV2})
...
# Arrays
reinterpret{T,CV<:Color}(::Type{CV}, A::Array{T,1}) = _reinterpret(CV, eltype(CV), A)
reinterpret{T,CV<:Color}(::Type{CV}, A::Array{T})   = _reinterpret(CV, eltype(CV), A)
_reinterpret{T,CV<:Color}(::Type{CV}, ::Type{Any}, A::Array{T}) =
    _reinterpret_array_cv(CV{T}, A)   # form 1 (turn into a form 2 call by filling in the element type of the array)
_reinterpret{T,CV<:Color}(::Type{CV}, TT::DataType, A::Array{T}) =
    _reinterpret_array_cv(CV, A)    # form 2
function _reinterpret_array_cv{T,CV<:Color}(::Type{CV}, A::Array{T})
    println(CV)
    if sizeof(T) == sizeof(CV)
...
_reinterpret_array_cv{CV<:Color}(::Type{CV}, A::Vector{UInt8}) =
    reinterpret(CV, A, (div(length(A), sizeof(CV)),))

# Images
function reinterpret{T,CV<:Color}(::Type{CV}, img::AbstractImageDirect{T})
...
convert{T<:Color,N}(::Type{Array{T}}, img::AbstractImageDirect{T,N}) = convert(Array{T,N}, img)
...
convert{C<:Color}(::Type{Image{C}}, img::Image{C}) = img
if !(VERSION < v"0.4.0-dev")
    convert{Cdest<:Color,Csrc<:Color}(::Type{Image{Cdest}}, img::Image{Csrc}) =
        copyproperties(img, _convert(Array{Cdest}, data(img)))  # FIXME when Julia issue ?? is fixed
end
convert{Cdest<:Color,Csrc<:Color}(::Type{Image{Cdest}}, img::AbstractImageDirect{Csrc}) =
    copyproperties(img, _convert(Array{Cdest}, data(img)))  # FIXME when Julia issue ?? is fixed
_convert{Cdest<:Color,Csrc<:Color,N}(::Type{Array{Cdest}}, img::AbstractArray{Csrc,N}) =
    _convert(Array{Cdest}, eltype(Cdest), img)     # FIXME when Julia issue ?? is fixed
_convert{Cdest<:Color,Csrc<:Color}(::Type{Array{Cdest}}, ::Type{Any}, img::AbstractArray{Csrc}) =
    convert(Array{Cdest{eltype(Csrc)}}, img)
_convert{Cdest<:Color,Csrc<:Color}(::Type{Array{Cdest}}, ::DataType, img::AbstractArray{Csrc}) =
    convert(Array{Cdest}, img)
convert{Cdest<:Color,Csrc<:Color,N}(::Type{Array{Cdest}}, img::AbstractImageDirect{Csrc,N}) =
    _convert(Array{Cdest}, convert(Array{Csrc,N}, img))

# Image{Color} -> Image{Numbers}
function separate{CV<:Color}(img::AbstractImage{CV})
    p = permutation_canonical(img)
...
function separate{CV<:Color}(A::AbstractArray{CV})
    T = eltype(CV)
    permutedims(reinterpret(T, A, tuple(length(CV), size(A)...)), [2:ndims(A)+1;1])
end
separate(A::AbstractArray) = A

# Image{Numbers} -> Image{Color} (the opposite of separate)
convert{C<:Color,T<:Fractional}(::Type{Image{C}}, img::Union(AbstractArray{T},AbstractImageDirect{T})) =
    _convert(Image{C}, eltype(C), img)
_convert{C<:Color,T<:Fractional}(::Type{Image{C}}, ::Type{Any}, img::Union(AbstractArray{T},AbstractImageDirect{T})) =
    _convert(Image{C{T}}, img)
_convert{C<:Color,T<:Fractional}(::Type{Image{C}}, ::DataType, img::Union(AbstractArray{T},AbstractImageDirect{T})) =
    _convert(Image{C}, img)
function _convert{C<:Color,T<:Fractional}(::Type{Image{C}}, img::Union(AbstractArray{T},AbstractImageDirect{T}))
    cd = colordim(img)
    if cd > 0
...
colorspace{C<:Color}(img::AbstractVector{C}) = string(C.name.name)
...
function getcolortype{T}(str::ASCIIString, ::Type{T})
    if haskey(colorspacedict, str)
        ...
        if endswith(str, "A")
            ...
            return AbstractAlphaColor{CV{T}, T}
...
colordim{C<:Color}(img::AbstractVector{C}) = 0
colordim{C<:Color}(img::AbstractMatrix{C}) = 0
colordim{C<:Color}(img::AbstractArray{C,3}) = 0
colordim{C<:Color}(img::AbstractImage{C}) = 0
