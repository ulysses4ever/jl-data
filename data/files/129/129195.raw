...
# with capital 4. states $0$ & $C_*$ are absorbing states, since in these cases the
# game ends. transition probs between non-absorbing states are 0.5; e.g.
# $P(s_n=5|s_{n-1}=4) = 1/2.$ and $P(s_n=0|s_{n-1}=0) = 1, P(s_n=c_*|s_{n-1}=c_*) = 1$
...
# $n_{ij}$ ... the number of states the MC is in state j
# when started in state i
N = inv(eye(t) - Q)

# $t_i$ ... expected number of steps till absorption when started in state i
t = N*ones(t)

# absorption probability B

# $b_{ij}$ ... prob, that start state i will be absorbed in state j
B = N*R
