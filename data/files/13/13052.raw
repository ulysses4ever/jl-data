...
function pixel_world_jacobian(wcs::WCSLIB.wcsprm, pix_loc::Array{Float64, 1};
    ...
    world_delt = maximum(abs(
      pixel_to_world(wcs, pix_loc + Float64[pixel_delt, pixel_delt]) - world_loc))

    world_loc_1 = world_loc + world_delt * Float64[1, 0]
    world_loc_2 = world_loc + world_delt * Float64[0, 1]

    hcat((world_to_pixel(wcs, world_loc_1) - pix_loc) ./ world_delt,
         (world_to_pixel(wcs, world_loc_2) - pix_loc) ./ world_delt)
end
