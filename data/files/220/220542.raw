## Blocked Gibbs sampler for DPM

@debug function dpmixture_blocked(init::GibbsInit)
    ...
    ## store stick-breaking draws
    dp_out.eta_out = zeros(Float64, J, M)
    
    ## count component membership
    ##counts = [count(z->z==j, label) for j in 1:J]
    njs = StatsBase.counts(label, 1:J)
    
    ## construct stick-breaking weights
    v = ones(J)
    for j in 1:J-1
        v[j] = rand( Beta( 1 + njs[j], alpha + sum(njs[j+1:J]) ) )
    end
    v[J] = 1
    ...
    ## begin the sampler
    if verbose @printf("Truncated J = %d\nInitial alpha = %f\nBegin sampler...", J, alpha) end
    ...
    for m in 1:M
        ...
        for j in 1:J
            ...
            if nj == 0                
                ## Sample empty component from prior
                
                Sigma_j = NobileWishart( rho, rho*R )
                SigmaInv_j = Sigma_j\I_3
                betas_j = beta_mu + chol(beta_V)'*randn(ktot)
        ...
        if alpha_a !=0
            ...
            ## update inverse scale
            b_star = alpha_b - sum( log( 1 - v[1:J-1] ) )
