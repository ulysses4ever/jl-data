...
function dpmixture_lppd(out::GibbsOut, x_new::Array{Float64,1}, z_new::Array{Float64,1};
                        ate_grid::Range=linspace(-10, 10, 100), sums=true)
    ...
    ## ate_grid::Range : points at which to evaluate ppd
    ...
    for i in 1:gridN
        ...
        for m in 1:M
            ...
            for j in 1:J
                ...
                ## ATE
                w[j] = pdfi( Distributions.Normal( xb, sqrt(g2) ), ate_grid[i] )
            ...
            ## ATE
            w[J+1] = pdfi( Distributions.Normal( xb, sqrt(g2)) , ate_grid[i] )
...
function dpmixture_lppd2(out::GibbsOut, x_new::Array{Float64,1}, z_new::Array{Float64,1};
    ...
    for m in 1:M
        ...
        for j in 1:J
            ...
            ## ATE
            append!( w, pdfi(Distributions.Normal( xb, sqrt(g2) ), ate_grid )) 
            
            ## TT
            hz = ( zb + (g1/g2)*(ate_grid - xb) )/sqrt( 1 - (g1^2)/g2 )
...
function dpmixture_ppd(out::GibbsOut, x_new::Array{Float64,1}, z_new::Array{Float64,1}; gridN=100, sums=true)
