...
f1l=feselect (fens,bfes,box=[-Inf,Inf,-Inf,Inf,0.0,0.0],inflate=tolerance);
f2l=feselect (fens,bfes,box=[-Inf,Inf,-Inf,Inf,-angslice,-angslice],inflate=tolerance);
...
list=fenodeselect(fens,distance=1.0+0.1/2^nref, from=[0. 0. 0.], inflate=tolerance);
fens.xyz[list,:]= JFinEALE.MeshUtilModule.ontosphere(fens.xyz[list,:],1.0);
...
l1=fenodeselect(fens,box=[-Inf Inf -Inf Inf 0.0 0.0],inflate=tolerance)
setebc!(u,l1,trues(length(l1)),l1*0+3,[0.0])    
l1=fenodeselect(fens,box=[-Inf Inf -Inf Inf 1.79  1.79],inflate=tolerance)
setebc!(u,l1,trues(length(l1)),l1*0+3,[0.0])    
applyebc!(u)
numberdofs!(u)
...
xsfemm = FEMMDeformationWinkler(FEMMBase(subset(bfes,[f1l,f2l]), GaussRule(order=3,dim=2)))

##
# We create the temperature field using the formula $T=r+z$.

# Temperature field
dT =NodalField(name="dT",data=reshape(sqrt(fens.xyz[:,1].^2+fens.xyz[:,2].^2)+fens.xyz[:,3],size(fens.xyz,1),1));


##
# And we are ready to assemble the system matrix. Both the elastic stiffness of the hexahedral elements ...
K =stiffness(mr, femm, geom, u)
...
l1 =fenodeselect(fens,box=JFFlt[-Inf  Inf -Inf  Inf 0.0 0.0],inflate=tolerance);
l2 =fenodeselect(fens,distance=1.0+0.1/2^nref,from=JFFlt[0.0 0.0 0.0],inflate=tolerance);
nA=intersect(l1,l2);
sA = mean(fld.values[nA])/phun("MEGA*Pa")
sAn = mean(fld.values[nA])/sigmaA
#println("Stress at point A: $(sA) i. e.  $( sAn*100  )% of reference value")
@test abs(sA -( -90.23804201195694))<1.0e-6
