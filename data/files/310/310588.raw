...
# sizes(x) = map(size, x)
mapf(functions, obj) = map(f->f(obj), functions)
...
function splitMatrixRows(m::AbstractMatrix, pct::Real)
...
stringfloats{T<:Number}(v::AbstractVector{T}, prec::Integer = 3) = string("[", join(map(x->stringfloat(x,prec), v), ", "), "]")
