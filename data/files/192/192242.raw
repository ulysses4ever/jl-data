...
println(self);
inter = nw-selfm;
getmotifstats(inter,motifidnoloop,motifcountnoloopbio)
println(motifcountnoloopbio)
# --- calculate average cooperativity over random networks
n = length(inter[:,1]);
ne = sum(inter .* inter);
np = int((ne+sum(inter))/2);

perftotal = 1000;           # perfect ensemble size
numinit = 1000;             # num of initial cond for fp determination
itotal=0
iperf=0

while (iperf<perftotal)
    ...
    if (isconnected(symadj(intersh))) # check if connected
        ...
        if (length(attlist) > 1)
            itotal += 1              # increase network count
...
writedlm("$dirname/loop.$f",motifcount);
writedlm("$dirname/noloop.$f",motifcountnoloop);
writedlm("$dirname/perf.$f",motifcountperf);
writedlm("$dirname/perfnl.$f",motifcountnoloopperf);
writedlm("$dirname/bio.$f",motifcountbio);
writedlm("$dirname/bionl.$f",motifcountnoloopbio);
