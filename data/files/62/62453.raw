...
(defgroup Shiki "Shiki Theme" :group appearance)

(defcustom Shiki:inactive-font-color "#c0c0c0"
  "Text color on title bars of inactive windows."
  :type color
  :group (appearance Shiki)
  :after-set after-setting-frame-option)

(defcustom Shiki:focused-font-color "#ffffff"
  "Text color on title bars of focused windows."
  :type color
  :group (appearance Shiki)
  :after-set after-setting-frame-option)

(defcustom Shiki:transients-can-iconify t
  "Transient windows can have iconify buttons."
  :type boolean
  :group (appearance Shiki)
  :after-set after-setting-frame-option)

(defcustom Shiki:transients-can-max t
  "Transient windows can have maximize/restore buttons."
  :type boolean
  :group (appearance Shiki)
  :after-set after-setting-frame-option)

(defcustom Shiki:show-resize-triangle nil
  "Show a resize triangle in the lower right corner of a window."
  :type boolean
  :group (appearance Shiki)
  :after-set after-setting-frame-option)

(defcustom Shiki:icon-mode 'if-available
...
(defcustom Shiki:title-text-offset 2
...
(define ltitle-colors `("#8d8481" "#4a4946"))

(define (font-colors w)
  (list Shiki:inactive-font-color Shiki:focused-font-color))
...
(define tframe-buttons `(
  ...
  ((cursor . left_ptr)
   ...
   (removable . t)
   (top-edge . -19)
   (hidden . ,(lambda (w) (not Shiki:transients-can-max)))
   (class . maximize-button)
   (background . ,max-restore-imgs))
  ...
  ((cursor . left_ptr)
   ...
   (removable . t)
   (top-edge . -19)
   (hidden . ,(lambda (w) (not Shiki:transients-can-iconify)))
   (class . iconify-button)
   (background . ,iconify-imgs))))
...
(define frame (append normal-base `(
  ((right-edge . 56)
   ...
   (left-edge . ,text-left-edge)
   (y-justify . ,(lambda (w) Shiki:title-text-offset))
   (class . title)
   (text . ,window-name)
  ...
  ((class . bottom-right-corner)
   (right-edge . 0)
   (bottom-edge . 0)
   (hidden . ,(lambda (w) (not Shiki:show-resize-triangle)))
...
(define shaped-frame (append shaped-base `(
  ((height . 22)
   ...
   (text . ,window-name)
   (x-justify . 8)
   (y-justify . ,(lambda (w) (+ 2 Shiki:title-text-offset)))
   (left-edge . ,text-left-edge)
   (foreground . ,font-colors))
...
(add-frame-style 'Shiki (lambda (w type) (case type
  ((shaped)           shaped-frame)
  ((shaded)           shaped-frame)
