...
(let*
  (
    ;(font (get-font "-adobe-helvetica-bold-r-normal-*-10-*-*-*-p-*-iso8859-1"))
    (custom-set-typed-variable (quote default-font) (quote ("Pango" . "Bitstream Vera Sans Bold 9")) (quote font))
    ...
    (title-width
      (lambda (w)
        ...
        (max 0 (+ 20 (min (- w-width 55) (text-width (window-name w) default-font)))))))
          
    (update-title-width
      (lambda (w)
        (if (eq (window-get w 'current-frame-style) 'Zeta) 
          (rebuild-frame w))))
