...
(let*
  (
    (font (get-font "-adobe-helvetica-bold-r-normal-*-12-*-*-*-p-*-iso8859-1"))
    ...
    (update-title-width
      (lambda (w)
        (if (eq (window-get w 'current-frame-style) 'BeOSish) 
          (rebuild-frame w))))
    ...
    (transient-frame `(
        ...
        ((background . ,bottom-left-image)
         ...
         (bottom-edge . -5)
         (class . bottom-left-corner))
        ((background . ,bottom-right-transient-image)
         (right-edge . -5)
         (bottom-edge . -5)
         (class . bottom-right-corner))
      ))
