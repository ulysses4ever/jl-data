...
function main(window)
	...
	vbox(
	h1("Bridges Loans (modèle classique)"),
	list([
         hbox("Set t0"   |> width(10em)             , slider(0:.01:1)  >>> t0_)       |> packacross(center) ,
         hbox("Set tx0"   |> width(10em)            , slider(0:.01:1) >>> tx0_)       , #|> packacross(center) ,
         hbox("Set duration"  |> width(10em)        , slider(0:.1:5)  >>> dur_)       , #|> packacross(center) ,
         hbox("Set draw period"  |> width(10em)     , slider(0:.1:5) >>> durt_)      , #|> packacross(center) ,
         hbox("Set currency USD"   |> width(10em)   , checkbox()              >>> cur_)       , #|> packacross(center) ,
         hbox("Set itraxx variation" |> width(10em) , slider(0.1:.01:4.) >>> itraxx_) , #|> packacross(center)
         ]) |> shrink(2.) ,
	hline()
	)

    lift(t0_, tx0_, dur_, durt_, cur_, itraxx_) do t0, tx0, dur, durt, cur, itraxx
        vbox(plot(x -> classic(x, t0, tx0, dur, durt, cur, itraxx), 0., 1.2,
             	  Scale.y_continuous(minvalue=0, maxvalue=1)) 
            ) |> grow(5.) |> pad(2em) # |> packitems(center)
    end
end
