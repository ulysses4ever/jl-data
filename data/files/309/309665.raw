module RNGTest
	...
	unsafe_store!(swrite, 0, 1)
        ...
        function WrappedRNG{RNG, T}(rng::RNG, ::Type{T}, isTRNG::Bool, fillarray = true, cache_size = 3*2^11 ÷ sizeof(T))
            ...
            fillcache(WrappedRNG{T, RNG}(rng, cache, fillarray,
                                         ...
                                         0, isTRNG)) # 0 is a dummy value, which will be set correctly by fillcache
        end
        ...
        wrap{T<:TestableNumbers}(rng::AbstractRNG, ::Type{T}, isTRNG::Bool) = WrappedRNG(rng, T, isTRNG)

        function fillcache{T}(g::WrappedRNG{T})
			if g.isTRNG
				println("isTRNG, length(g.cache) = $(length(g.cache)),length(g.vals) = $(length(g.vals)) ")
				
				# include the code for the TRNG call here
				# 
				# ideas: 
				# 	1) request length(g.cache) numbers from TRNG 
				#	2) generate the seeds from TRNG then pass to srand(seed), then rand().
				#
				#
		...
		function Unif01(f::Function, genname)
			...
			return b;
                ...
                function Unif01{T<:Integer}(g::WrappedRNG{T}, genname)
                    ...
                    b = new(ccall((:unif01_CreateExternGenBits, libtestu01), Ptr{Void}, (Ptr{Uint8}, Ptr{Void}), genname, cf), UInt32)
                	b.isTRNG=false;
					return b;
				end
				
				
				# include the code for the TRNG load file call here
				# 
				# ideas: 
				#	1) try gen = ufile_CreateReadText (filename, 10 * MILLION);
				#
				
				function Unif01(filename::String,len::Int)
					if contains(filename,".txt")
						#
						# applies SmallCrush to a RNG given as a text file of floating-point numbers in [0, 1); the file requires slightly less than 51 320 000 random numbers. Some of these tests assume that gen returns at least 30 bits of resolution; if this is not the case, then the generator is most likely to fail these particular tests.
						#
						b = new(ccall((:ufile_CreateReadText, libtestu01), Ptr{Void}, (Ptr{Char}, Clong), pointer(filename),len), Float64)
					else
						
						b = new(ccall((:ufile_CreateReadBin, libtestu01), Ptr{Void}, (Ptr{Char}, Clong), pointer(filename),len), UInt32)
					end
					b.isTRNG=true;
					return b
				end
	...
	function delete(obj::Unif01)
		if obj.isTRNG
			ccall((:ufile_DeleteReadText, libtestu01), Void, (Ptr{Void},), obj.ptr)
			obj.isTRNG= false
		else
	...
	## smarsa using a file
	function smarsa_BirthdaySpacings(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, d::Integer, t::Integer, p::Integer)	 	
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResPoisson()
	 	ccall((:smarsa_BirthdaySpacings, libtestu01), Void,
			(Ptr{Void}, Ptr{Void}, Clong, Clong,
	 		 Cint, Clong, Cint, Cint),
		...
		#ccall((:ufile_DeleteReadBin, libtestu01),Void,(Ptr{Void},), unif01.ptr)
	 	delete(unif01)
	 	return pvalue(sres)
	end
	function smarsa_MatrixRank(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, s::Integer, L::Integer, k::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResChi2(N)
		ccall((:smarsa_MatrixRank, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	## sknuth using a file
	function sknuth_Collision(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, d::Integer, t::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = KnuthRes2()
		ccall((:sknuth_Collision, libtestu01), Void, 
			(Ptr{Void}, Ptr{Void}, Clong, Clong,
	...
	function sknuth_Gap(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, Alpha::Real, Beta::Real)
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResChi2(N)
		ccall((:sknuth_Gap, libtestu01), Void, 
			(Ptr{Void}, Ptr{Void}, Clong, Clong,
	...
	function sknuth_SimpPoker(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, d::Integer, k::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResChi2(N)
		ccall((:sknuth_SimpPoker, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	function sknuth_CouponCollector(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, d::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResChi2(N)
		ccall((:sknuth_CouponCollector, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	function sknuth_MaxOft(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, d::Integer, t::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = KnuthRes1(N)
		ccall((:sknuth_MaxOft, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	## svaria using a file
	function svaria_WeightDistrib(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, k::Integer, alpha::Real, beta::Real)
		unif01 = Unif01(filename, NumberOfRN);
		sres = ResChi2(N)
		ccall((:svaria_WeightDistrib, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	## sstring using a file
	function sstring_HammingIndep(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, s::Integer, L::Integer, d::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = StringRes()
		ccall((:sstring_HammingIndep, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	## swalk using a file
	function swalk_RandomWalk1(filename::String, NumberOfRN::Integer, N::Integer, n::Integer, r::Integer, s::Integer, L0::Integer, L1::Integer)
		unif01 = Unif01(filename, NumberOfRN);
		sres = WalkRes(N)
		ccall((:swalk_RandomWalk1, libtestu01), Void, 
			 (Ptr{Void}, Ptr{Void}, Clong, Clong, 
	...
	#
	#
	### file dependand test batteries
	#
	#
	
	function smallcrushfileJulia(filename::String, NumberOfRN::Integer)
 		testnames = [g->smarsa_BirthdaySpacings(g, NumberOfRN, 1, 5000000, 0, 1073741824, 2, 1),
 					 g->sknuth_Collision(g, NumberOfRN, 1, 5000000, 0, 65536, 2),
 					 g->sknuth_Gap(g, NumberOfRN, 1, 200000, 22, 0.0, .00390625),
 					 g->sknuth_SimpPoker(g, NumberOfRN, 1, 400000, 24, 64, 64),
 					 g->sknuth_CouponCollector(g, NumberOfRN, 1, 500000, 26, 16),
 					 g->sknuth_MaxOft(g, NumberOfRN, 1, 2000000, 0, 100000, 6),
 					 g->svaria_WeightDistrib(g, NumberOfRN, 1, 200000, 27, 256, 0.0, 0.125),
 					 g->smarsa_MatrixRank(g, NumberOfRN, 1, 20000, 20, 10, 60, 60),
 					 g->sstring_HammingIndep(g, NumberOfRN, 1, 500000, 20, 10, 300, 0)[:Mean],
 					 g->swalk_RandomWalk1(g, NumberOfRN, 1, 1000000, 0, 30, 150, 150)]
 		return pmap(t->t(filename), testnames)
	end	
