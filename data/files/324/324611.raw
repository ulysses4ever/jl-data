...
#----------------------------------------------------------------
fileNamePrefix = "../data/local/output_set_2_label_3_recoded_Likert_3_pt_iterations"
fileName = "../data/local/output_set_2_label_3_recoded_Likert_3.csv";
fid = open( fileName, "r");
...
K = int( H[ 3 ] );
R = int( H[ 4 ] );
maxIter = int( H[ 5 ] );  # not used
scaleGamma = int( H[ 6 ] ); # not used
shapeGamma = int( H[ 7 ] ); # not used
vFactorGamma = int( H[ 8 ] ); # not used
...
fid = open( string(fileNamePrefix, "meanPhi1.csv" ), "w" );
writecsv( fid, squeeze( meanPhi[ : ,1, :], 2) );
close( fid );

fid = open( string(fileNamePrefix, "meanPhi2.csv" ), "w" );
writecsv( fid, squeeze( meanPhi[ : ,2, :], 2) );

close( fid );fid = open( string(fileNamePrefix, "meanPhi3.csv" ), "w" );
writecsv( fid, squeeze( meanPhi[ : ,3, :], 2) );
close( fid );

fid = open( string(fileNamePrefix, "meanPZmk.csv" ), "w" );
