...
# Includes partially observed data matrix Lnm
# Created by Salvador Ruiz Correa 02/04/15
# TODO: ALPHA used as a global variable in some functions: change this
###############################################################
...
# ------------------------------------------------------------------
# gibbsSampler() - partial Lnm data matrix
# ------------------------------------------------------------------
...
for iter = 1 : maxIter
    ...
    if ind[ vars ] <= count0  # update Lnm
    	...
    	if L[ n, m ] == 1
			...
			totProb = 0.;
    	    for r = 1 : R
    	    	probS[ r ] = ( Snkr[ n, Zm[ m ], r ] + GAMMA ) / ( Snk[ n, Zm[ m ] ] + RGAMMA );
    	    	totProb += probS[ r ];
    	    end
...
fileName = "../data/non_local_output_set_2_label_1_recoded_Likert_3.csv";
fid = open( fileName, "r");
...
Par = gsParameters( seed, BURNIN, maxIter, shapeAlpha, scaleAlpha, shapeGamma, scaleGamma, vFactorGamma );
Model = gsModel( N, M, K, R, ALPHA, GAMMA, rand( int64( 1 : K ), M ) );

idL = find(Lnm .== 0 );
L = zeros( Int, size( Lnm ) );
L[ idL ] = 1;


gammaAccRate, logP = gibbsSampler( Lnm, L, idL, Model, Par, true, false, "" ); 


fileName = "../data/logP.csv";
fid = open( fileName, "w");
writecsv( fid, logP' );
close( fid );
...
Par.seed += 1;
gammaAccRate, logP = gibbsSampler( Lnm, L, idL, Model, Par, false, true, "../data/iterations" ); 

ipse, imse = estimateVariance( logP )

fileName = "../data/logP2.csv";
fid = open( fileName, "w");
writecsv( fid, logP' );
close( fid );
