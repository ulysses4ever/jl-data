...
function gsParallelTempering( pTM::gsPTModel, pTMPar::gsPTModelPar, pTPar::gsPTPar, bInit::Bool, bWrite::Bool, fileNamePrefix::String, bVerbose::Bool )
...
if bInit							# random state initialization					
	Model = gsModel(	pTM.N, 
						...
						Beta );
for t = 1 : T 
	gibbsSampler( Lnm, Model, Par, true, t )
end
...
for iter =  1 : pTPar.maxIter
	if rand() < SWAP_PROB || iter == 1  	# Update all states
		it +=1
		for t = 1 : T
			logP[ it, t ] = gibbsSampler( Lnm, Model, Par, false, t );	
		end
...
return exchangeRate, Model.GAMMA, Model.Zm;
...
if bBurnIn # compute BURNINN samples
	...
	@printf( "\n------------------------------------------");
	@printf( STDOUT, "\nBurnin ..." );
	exchangeRate, GAMMA, Zm = gsParallelTempering( pTM, pTMPar, pTPar, true, true, fileNameTestPrefix, bVerbose );
	...
	exchangeRate, GAMMA, Zm = gsParallelTempering( pTM, pTMPar, pTPar, false, true, fileNamePrefix, bVerbose );
