...
goldenLS = (f,df,dk,dfk,xk,pk) -> golden(f,xk,pk,0.0,5.0)
...
# steepest descent
x1,flag1,his1 = sd(f,df,x0,maxIter=1000,atol=1e-6,out=0,lineSearch=goldenLS)
@test norm(x1)<1e-6*norm(x0)
x2,flag2,his2 = sd(f,df,x0,maxIter=1000,atol=1e-6,out=0, lineSearch=goldenLS)
@test norm(x2)<1e-6

# nlcg
x1,flag1,his1 = nlcg(f,df,x0,maxIter=1000,atol=1e-6,out=0)
@test norm(x1)<1e-6*norm(x0)
x2,flag2,his2 = nlcg(f,df,x0,maxIter=1000,atol=1e-6,out=0, lineSearch=goldenLS)
@test norm(x2)<1e-6

# bfgs
x1,flag1,his1 = bfgs(f,df,x0,maxIter=1000,atol=1e-6,out=0)
@test norm(x1)<1e-6*norm(x0)
x2,flag2,his2 = bfgs(f,df,x0,maxIter=1000,atol=1e-6,out=0, lineSearch=goldenLS)
@test norm(x2)<1e-6
