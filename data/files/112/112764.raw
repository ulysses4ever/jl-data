...
function backTrackSearch(alpha_guess,Xk,dk,F,gamma,delta,rhok)
...
# end
    ...
    alpha = alpha_guess;
    while (compF(F,(Xk+alpha.*dk))>compF(F,(Xk))-gamma*alpha^2*(norm(dk))^2) 
    ...
    alpha1 = alpha;
    F1     = compF(F,(Xk+alpha1.*dk))-(compF(F,(Xk))-gamma*alpha1^2*(norm(dk))^2);
    ...
    alpha2 = -alpha;
    F2     = compF(F,(Xk+alpha2.*dk))-(compF(F,(Xk))-gamma*alpha2^2*(norm(dk))^2);
