...
type Linear  <: BasisFunction end
type Quadratic <: BasisFunction end
...
function NGrid{BT<:BasisFunction}(L::Vector{Int},bounds::Array{Float64} = [0,1]*ones(1,length(L));B::Type{BT}=Linear)
    R = vec(diff(bounds,1))
    grid,ind,weights = SmolyakGrid(L,R)
    ...
    c_dM = map(x->cc_dM(Int(x)),coverings)
	...
	lev_M = map(i->Int16(cc_M(Int(i))),ind)::Array{Int16,2}
