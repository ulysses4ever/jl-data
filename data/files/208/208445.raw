...
function Vasicek(name::Symbol,
    ...
    for mc = 1:n_mc
        yield[mc,:,1] = yieldvasicek(mc, init, a, yield_infty, tf, noise) 
    end 
    cpnt_id = Dict(cpnt, 1:length(cpnt))
    Vasicek(name, cpnt, cpnt_id, init, a, yield_infty,
            cov, noise, n, yield, n_mc, dt, n_p )
end
...
function Vasicek(name::Symbol,
    ...
    noise =
        reshape( rand( Normal(0,sqrt(cov)), n_mc*tf.n_p ),
                n_mc, tf.n_p )
    Vasicek( name, cpnt, init, a, yield_infty,
            tf, cov, noise )
end
...
function yieldeoc(me::Vasicek, n_mc::Int, tf::TimeFrame, yield_init_c::Float64)
...
function forwardbop(me::Vasicek, mc::Int, t::Int, delta_t::Int)
    ...
    logA =
        (B-delta_t)*(me.a^2*me.yield_infty - 0.5me.cov) /me.a^2 -
    me.cov * B^2 / 4me.a
