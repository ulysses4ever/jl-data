function covNNone(hyp::Vector, x::Matrix, z::Matrix, i)
    # Neural network covariance function with a single parameter
    # for the distance measure. The covariance function is parameterized as:
    #
    # k(x^p, x^q) = sf2 * asin(x^p' * P * x^q /
    #                          sqrt[(1 + x^p' * P * x^p) * (1 + x^q' * P * x^q)])
    #
