...
#*-------------------------------------------------------------------
type Section_Column_EffEmp
	Section_Column_EffEmp()=begin
		PP=outers.PP
		NComp=outers.NComp
		new(
...
export Section_Column_EffEmp
function set(in::Section_Column_EffEmp)
	top = (NTrays-1)*(1-topdown)/2+1
	 bot = NTrays/top
	 
end
function setEquationFlow(in::Section_Column_EffEmp)
	if NComp > 2 
		addEquation(1)
		addEquation(2)
	else
...
function atributes(in::Section_Column_EffEmp,_::Dict{Symbol,Any})
...
Section_Column_EffEmp(_::Dict{Symbol,Any})=begin
	newModel=Section_Column_EffEmp()
