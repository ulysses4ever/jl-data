  ...
  if length(shape_test.bounds[:,1])>=3
    for k = 1:length(shape.bounds[:,1])
      for i = 0:length(shape_test.bounds[:,1])-3
        ...
        if s>=0 && t>=0 && 1-s-t>=0
          return "Found a collision (half-quad inside shape_test)"
        end
      end
    end
  end
  println("end of half-quad inside shape_test")

  println()

  println("begin of shape_test inside shape")
  for i = 0:length(shape.bounds[:,1])-3
    for k = 1:length(shape_test.bounds[:,1])
      ...
      s = 1/(2*Area)*(points_shape[1].y*points_shape[3+i].x - points_shape[1].x*points_shape[3+i].y + (points_shape[3+i].y - points_shape[1].y)*points_shape_test[k].x + (points_shape[1].x - points_shape[3+i].x)*points_shape_test[k].y)

      t = 1/(2*Area)*(points_shape[1].x*points_shape[2+i].y - points_shape[1].y*points_shape[2+i].x + (points_shape[1].y - points_shape[2+i].y)*points_shape_test[k].x + (points_shape[2+i].x - points_shape[1].x)*points_shape_test[k].y);
