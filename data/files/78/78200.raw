...
function translate_rna(seq)
    ...
    ct=split(codon_table)
    codon_mapping = [k=>v for (k,v) in zip(ct[1:2:end],ct[2:2:end])]
    transcribed = [codon_mapping[seq[i:i+2]] for i in 1:3:length(seq)-2]
    stop = findfirst(transcribed,"Stop")
    println(stop)
    join(transcribed[1:stop-1],"")
end
