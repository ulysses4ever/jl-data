#==============================================================================
Problem
Figure 2. The probability of any outcome (leaf) in a probability tree diagram 
is given by the product of probabilities from the start of the tree to the 
outcome. For example, the probability that X is blue and Y is blue is equal 
to (2/5)(1/4), or 1/10.
...
ball, then the probability of each of the two outcomes is given by 
...
function inherit_dominant(homo_dom::Int, hetero::Int, homo_rec::Int)
    counts = Float64[homo_dom,hetero,homo_rec]
    tot = sum(counts)
    ...
    prob = (1/(tot*(tot-1)))*(
           ...
           2*counts[1]*counts[2] +
           2*counts[1]*counts[3] + 
        0.75*counts[2]*(counts[2]-1) +
       2*0.5*counts[2]*counts[3]
    )
end
